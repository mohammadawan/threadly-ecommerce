    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      {orders.length === 0 ? (
        <div className="text-center py-16">
          ...
          <Link to="/products" className="bg-black text-white px-6 py-2 rounded-xl text-sm font-medium hover:bg-gray-800">Start Shopping</Link>
        </div>
      ) : (
        <div className="space-y-4">
          {orders.map((order) => (
            <Link key={order._id} to={`/orders/${order._id}`} className="block bg-white border border-gray-200 rounded-xl p-5 hover:shadow-md transition">
